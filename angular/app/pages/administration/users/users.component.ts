...
export class UsersComponent extends MainBaseComponent<BaseDto, FilterUserDto, DeleteUserRangeDto> implements OnInit {
  ...
  constructor(
    ...
    private userServiceService: AdministrationService,
    ...
    shareService.changeEmitted$.subscribe( dto => {
      ...
      if(!dto.ids.includes(COMPONENT_IDS.USER_COMPONENT)) {
        return;
      }
    });
  ...
  getPaginatedList(){
    ...
    this.userServiceService.filter(this.filterDto).subscribe( (subscribe) =>{
     // debugger;
  ...
  onDelete(id: string){
    this.userServiceService.delete(id).subscribe( (subscribe: any) =>{
  ...
  onDeleteRange(){
    ...
    this.userServiceService.deleteRange(this.deleteRangeDto).subscribe((sub: any)=>{
      if(sub){
